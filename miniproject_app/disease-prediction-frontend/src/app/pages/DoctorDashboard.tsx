...
export default function DoctorDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch availability
        const availRes = await axios.get('http://127.0.0.1:8000/api/users/availability/', {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleAvailabilityToggle = async (checked: boolean) => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      await axios.post('http://127.0.0.1:8000/api/users/availability/',
  ...
  const pendingRequests: any[] = []; // Left blank for now as there's no backend model for "pending" requests in this project, only chatrooms
  const acceptedPatients = chatRooms;
                ...
                >
                  ...
                  <div className="mb-3">
                    <p className="text-sm text-gray-600">
                      Predicted: <span className="font-semibold text-gray-900">{request.predictedDisease}</span>
                    </p>
            ...
            <div className="space-y-4">
              {loading ? (
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <h3 className="font-semibold text-gray-900">{patient.patient_name || `Patient ${patient.patient}`}</h3>
                      <p className="text-sm text-gray-600">Assigned Patient</p>
                    </div>
                  ...
                  <Button
                    ...
                    variant="outline"
                    className="w-full"
                    onClick={() => navigate('/doctor/chat', { state: { roomId: patient.id, chatPartnerName: patient.patient_name || `Patient ${patient.patient}` } })}
                  >
          ...
          <Card className="p-6 shadow-md border-0">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3 p-3 rounded-lg bg-gray-50">
                ...
                <div>
                  <p className="font-semibold text-gray-900">9:00 AM - 10:00 AM</p>
                  <p className="text-sm text-gray-600">General Checkup - Sarah Miller</p>
                </div>
              </div>
              <div className="flex items-center gap-3 p-3 rounded-lg bg-gray-50">
                ...
                <div>
                  <p className="font-semibold text-gray-900">11:00 AM - 12:00 PM</p>
                  <p className="text-sm text-gray-600">Follow-up - David Chen</p>
                </div>
              </div>
            </div>
          </Card>
